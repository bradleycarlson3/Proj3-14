...
export default {
  setup(_, { emit }) {
    ...
    const v$ = useVuelidate({
      event: {
        name: { required },
        date: { required, validDate: value => !isNaN(new Date(value)) }
      },
    }, event);

    // Load event, attendees, and services data
    onMounted(async () => {
      try {
        ...
        event.value = { ...eventResponse, date: new Date(eventResponse.date).toISOString().substring(0, 10) };
    ...
    const submitEventUpdate = async () => {
      ...
      try {
        await updateEvent(this.$route.params.id, event.value);
...
<!-- Options API script section for reference
<script>
