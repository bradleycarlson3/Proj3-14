...
export default {
  setup() {
    ...
    const handleSubmitForm = async () => {
      if (searchBy.value === 'Event Name' && name.value) {
        try {
          ...
          toast.error('Error searching events by name', error.message);
        }
      } else if (searchBy.value === 'Event Date' && eventDate.value) {
        try {
          const formattedDate = eventDate.value.toISOString().substring(0, 10);
          const query = { searchBy: 'date', eventDate: formattedDate };
          const response = await searchEvents(query);
          events.value = response;
        } catch (error) {
          toast.error('Error searching events by date', error.message);
        }
      }
    };
    ...
    return {
      events, searchBy, name, eventDate, hoverId, loadData, handleSubmitForm, formatDate
    };
  },
}
</script>
