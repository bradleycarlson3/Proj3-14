...
<!--Modified script section to reflect Composition API-->
<script>
...
export default {
  setup() {
    ...
    const submitServiceUpdate = async () => {
      ...
      try {
        const serviceId = router.currentRoute.value.params.id;
        const response = await updateService(serviceId, service.value);
    ...
    return { service, events, hoverId, user, v$, formatDate, submitServiceUpdate, submitDeleteService };
  },
}
</script>
